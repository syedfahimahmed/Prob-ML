...
class LogisticRegression:
    ...
    def fit(self, X, y, weights="zeros"):
        ...
        # Define gradient of negative log-likelihood function
        def neg_log_likelihood_grad(w):
            y_pred = sigmoid(X @ w)
            return X.T @ (y_pred - y) + w
        
        # Define Hessian of negative log-likelihood function
        def neg_log_likelihood_hess(w):
            y_pred = sigmoid(X @ w)
        ...
        for i in range(self.max_iter):
            ...
            grad = neg_log_likelihood_grad(self.weights)
            hess = neg_log_likelihood_hess(self.weights)
